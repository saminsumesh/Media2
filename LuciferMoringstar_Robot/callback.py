...
# ---------- 📁 [ | 𝗚𝗘𝗧 𝗙𝗜𝗟𝗘𝗦 | ] 📁 ---------- #
            ...
            try:
                if FORCES_SUB and not await is_subscribed(client, query):
                    ...
                    await message.reply_text(text=LuciferMoringstar.CP_DELETE.format(mention= query.message.from_user.menntion, title= '' if title is None else title, size='' if size is None else size, caption='' if files.caption is None else files.caption), reply_markup=reply_markup)
